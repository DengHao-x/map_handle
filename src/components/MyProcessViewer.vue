  ...
  export default {
    ...
    watch: {
      ...
      taskData: function (newTaskListData) {
        ...
        console.log(newTaskListData,'taskData111')
        this.createNewDiagram(this.xml);
      }
    },
    methods: {
      ...
      async highlightDiagram() {
        ...
        // debugger
        console.log(this.bpmnModeler.getDefinitions().rootElements[0].flowElements,'111');
        this.bpmnModeler.getDefinitions().rootElements[0].flowElements?.forEach(n => {
          let activity = activityList.find(m => {
            console.log(m.key===n.id,'m.key')
            return m.key === n.id
          }) // 找到对应的活动
          ...
          console.log(n.$type==='bpmn:Task','n.$type')
          if(n.$type==='bpmn:Task'){
             // 处理用户任务的高亮
             const task = this.taskData.find(m => {
              console.log(m,'mm')
              return m.id === activity.taskId
             }); // 找到活动对应的 taskId
             console.log(this.taskData,task,'找到活动对应的 taskId')
             if (!task) {
              return;
            }
